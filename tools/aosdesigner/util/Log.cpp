	...
	namespace
	{
		...
		private:
			...
			static boost::filesystem::path generate_log_file_path()
			{
				...
				const auto filename = boost::format( "aosdesigner_%1$04i%2$02i%3$02i_%4$02i%5$02i%6$02i.log" ) % date.year() % int(date.month()) % date.day() % time.hours() % time.minutes() % time.seconds();

				if( !boost::filesystem::exists( path::LOG_FILE_DIR ) )
				{
					boost::filesystem::create_directories( path::LOG_FILE_DIR );
				}

				return path::LOG_FILE_DIR / boost::filesystem::path( filename.str() );
			}
